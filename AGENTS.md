...
- **PTZ slider updates use PatchSignals**: `handlePTZ` sends `sse.MarshalAndPatchSignals(status.PTZValues)` on success (~20 bytes) instead of `PatchElementTempl` (~4KB full panel). On error, it still sends full panel HTML for the error banner. The slider value and radar position update reactively from the `$pan`/`$tilt`/`$zoom` signals.
- **`data-indicator` loading states**: All action buttons (mode cards, audio segments, toggles, center, sync, probe) have `data-indicator="loading"` + `data-class:btn-loading="$loading"`. DataStar's built-in indicator toggles the shared `$loading` signal during SSE round-trips, showing the `.btn-loading` CSS class (opacity 0.7, cursor wait, pointer-events none) and preventing concurrent operations.
