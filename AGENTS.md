...
- **`vendorHash` is shared between `flake.nix` and `package.nix`**: Both files must be kept in sync when dependencies change. The hash is a SHA of module tarballs downloaded via `proxyVendor = true`, not of vendored source.
