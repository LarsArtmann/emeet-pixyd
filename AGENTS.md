...
- **SSE connection indicator + offline banner**: `app.js` listens to DataStar's document-level `datastar-fetch` custom events (filtered to `detail.el === document.body`, which is the SSE element). On `datastar-patch-elements` (data flowing) → indicator shows green (`.connected`). On `started` → yellow (default, connecting). On `retrying`/`retries-failed` → red (`.disconnected`) + `#offline-banner` shows. Both `#sse-indicator` and `#offline-banner` are in `page()` OUTSIDE `#status-panel` so DataStar morphs don't reset their state.
