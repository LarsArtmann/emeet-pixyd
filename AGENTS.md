...
**Updated:** 2026-07-04 (UI/UX overhaul — preview hero, mode cards, PTZ radar, snapshot, shortcut legend)
...
- **PTZ radar**: `ptzRadar(pan, tilt)` template renders a 120px circular position indicator with crosshair, inner ring, center dot, and a glowing position dot. Dot position set via CSS custom properties `--pan-x` and `--pan-y` (percentages computed from pan/tilt ranges). The radar lives inside the PTZ card in the panel. `app.js` `updateRadar()` updates the dot live as sliders move (called on `input` event and on error revert). Server re-renders correct position on panel refresh.
