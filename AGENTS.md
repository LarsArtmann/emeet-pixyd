...
| `handlers.go`                 | HTTP routing, web handlers, DataStar SSE response rendering                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `sse.go`                      | SSE-only: `Broadcaster` (thread-safe fan-out for state-change notifications). Wire-format layer deleted — DataStar SDK (`datastar-go`) handles SSE event formatting. (~85 lines)                                                                                                                                                                                                                                                                                          |
...
| `static/`                     | Frontend assets (`app.js`, `style.css`, `datastar.js`) — embedded via `//go:embed`. DataStar v1.0.2 JS runtime (34 KB) served as ES module at `/static/datastar.js`.                                                                                                                                                                                                                                                                                                      |
...
| `pixy_simulator_impl_test.go` | 39 tests: config/commit validation, query round-trips, failure injection, circuit breaker (accumulation, reset, commit-failure), 200ms sleep timing, `pixyConfig`/`pixyCommit` byte-layout tables, daemon integration (setTracking/setAudio/setGesture), syncState reads, protocol bytes for all interfaces                                                                                                                                                                  |
...
- **HID protocol simulator**: `pixy_simulator_test.go` provides `pixySimulator` — a protocol-faithful `HIDDevice` implementation that validates every outgoing byte against the PIXY wire protocol. Unlike `fakeHIDDevice` (which blindly records bytes), the simulator enforces config→commit sequencing, validates mode bytes per interface, and generates protocol-valid query responses that round-trip through `parseHIDResponse`. `withPixySimulator()` wires it into the daemon while keeping real `setTracking`/`setAudio`/`setGesture` methods (exercises the full `setDeviceState` → `Send` → protocol validation path, including the 200ms sleep). Supports failure injection: `sendErr` (all Sends), `commitErr` (commit-only), `sendRecvErr`, `nilResponse` (timeout), `corruptResp` (garbage). State accessors (`Tracking()`, `Audio()`, `Gesture()`), recording (`SentReports()`, `SentTimestamps()`, `Queries()`), and `pixyProtocolState` as a `*` pointer (not value) enable test assertions including circuit breaker accumulation/reset and 200ms timing verification.
...
- Predefined test options: `withInCall()`, `withAutoOff()`, `withCameraInUse()`, `withNotifyCalled()`, `withNotifyMessages()`, `withFindSource()`, `withCaptureTracking()`, `withCaptureAudio()`, `withCaptureGesture()`, `withCaptureGestureArg()`, `withCaptureCenter()`, `withNoopV4L2()`, `withNoopTracking()`, `withNoopAudio()`, `withDebounceCount()`, `withPixySimulator()` (returns `(*pixySimulator, testDaemonOption)` — protocol-faithful HID device with byte validation, failure injection, and recording)
...
- **DataStar panel morphing**: The `#status-panel` div is morphed via DataStar's `PatchElementTempl` SSE events. DataStar morphs elements by ID automatically (using idiomorph) — no swap directive needed. The panel is driven by `data-init="@get('/api/events', {openWhenHidden: true})"` on `<body>`, which establishes a persistent SSE connection.
- **PTZ slider updates use PatchSignals**: `handlePTZ` sends `sse.MarshalAndPatchSignals(status.PTZValues)` on success (~20 bytes) instead of `PatchElementTempl` (~4KB full panel). On error, it still sends full panel HTML for the error banner. The slider value display, radar position, and slider thumb all update reactively: `data-bind` provides signal→element sync (slider thumb reflects external changes), `data-on:input` provides element→signal sync (user drag updates signals), and `data-style`/`data-text` provide reactive radar dot and value display.
- **`data-indicator` loading states (shared `$loading` is intentional)**: All action buttons (mode cards, audio segments, toggles, center, sync, probe) have `data-indicator="loading"` + `data-class:btn-loading="$loading"`. The **shared** `$loading` signal is a deliberate design choice: a hardware daemon must serialize operations (HID has a 200ms protocol sleep, V4L2 has a mutex), so showing loading on ALL buttons during any operation prevents concurrent requests and communicates the serialized nature to the user. DataStar's indicator increments/decrements a counter, so nested fetches correctly stack.
- **Scripts at end of body**: DataStar (`type="module"`) and `app.js` are loaded at the end of `<body>` (not in `<head>`) because `app.js` accesses `document.body` at load which would be `null` in `<head>`.
...
- **`go-branded-id` committed-binary workaround** (TEMPORARY): `go-branded-id@v0.5.0` ships a committed compiled `namer` ELF binary that embeds nix store paths (`/nix/store/.../go-1.26.5`). nix's fixed-output derivation invariant forbids the `go-modules` FOD from referencing store paths, so the build fails with `fixed-output derivations must not reference store paths`. Fix is **in-sandbox only** (committed `go.mod`/`go.sum` stay canonical so GitHub Actions `go test` against the real proxy is unaffected): `flake.nix` defines `goBrandedSrc = fetchFromGitHub v0.5.0` with `postFetch = "rm -f $out/namer"`, and both derivations' `preBuild` run `go mod edit -replace=github.com/larsartmann/go-branded-id@v0.5.0=${goBrandedSrc}`. With a local `replace`, `go mod download` skips the poisoned module entirely (it never enters the FOD cache). This is invisible to `go` CI because the binary in the real proxy module is harmless to compilation — only the nix FOD reference check rejected it. The go-branded-id repo has stopped tracking the binary (commit `c29a034`). **Follow-up**: once go-branded-id publishes a version without the committed binary, remove `goBrandedSrc`/`replaceBrandedId` from `flake.nix` + `package.nix`, bump `go.mod`, and recompute `vendorHash`.
- **Overlay uses `self.packages`, not `callPackage`**: `overlays.default` is `final: _prev: { emeet-pixyd = self.packages.${final.system}.emeet-pixyd; }`. It does NOT re-`callPackage ./package.nix` because the package now requires the `goBrandedSrc`/`replaceBrandedId` args (which live in `perSystem`'s scope, unreachable from the flake-level overlay). Referencing the already-built flake package sidesteps that entirely. The NixOS module depends on this via `mkPackageOption pkgs "emeet-pixyd"` (`modules/nixos.nix`).
...
- **Live UI updates via DataStar SSE**: `/api/events` endpoint uses `datastar.NewSSE(w, r)` + `Broadcaster` for thread-safe fan-out. The handler sends the initial `statusPanel` as a `PatchElementTempl` on connect, then patches on every state-change broadcast. DataStar's `@get()` action establishes the persistent SSE connection via `data-init` on `<body>` — built-in retry with exponential backoff. `app.js` no longer manages SSE connections.
- **State-change broadcasting**: `broadcastStateChanged()` calls `d.broadcaster.Broadcast()` (zero-payload notification) after every state mutation (`setDeviceState`, `syncState`, `handleCallStart`, `handleCallEnd`, `applyProbeResultLocked`). The local `Broadcaster` is a thread-safe fan-out hub with `chan struct{}` channels — non-blocking sends drop events for slow clients (buffered channel per subscriber). The `Daemon.broadcaster` field (`*Broadcaster`) is initialized in both `NewDaemon()` and `newTestDaemon()`.
...
- **`vendorHash` is shared between `flake.nix` and `package.nix`**: Both files must be kept in sync when dependencies change. The hash is a SHA of module tarballs downloaded via `proxyVendor = true`, not of vendored source. All dependencies are on the public Go module proxy (HTMX and cqrs-htmx replaced by DataStar — no private repos).
- **UI layout — preview as hero**: The camera preview is full-width above the status panel (not in a 2-column grid). The preview is rendered once in `page()` and does NOT re-render on DataStar morphs (only `#status-panel` morphs). The preview online/offline state is handled by `app.js` error recovery, not server-side morphs. When offline, a placeholder card replaces the PTZ section inside the panel.
...
- **Audio segmented control**: Three `audioSegment` buttons inside a `.segmented` container (was `audioBtn`). Active segment highlighted with accent color. Each button posts `@post('/api/audio/<mode>')` — mode is in the URL path, not form data.
- **PTZ radar**: `ptzRadar(pan, tilt, zoom)` template renders a 120px circular position indicator with crosshair, inner ring, zoom ring, center dot, and a glowing position dot. Dot position set via CSS custom properties `--pan-x` and `--pan-y` (percentages computed from pan/tilt ranges). Zoom ring size set via `--zoom-pct` (scales 30%→80% of radar diameter). The radar lives inside the PTZ card in the panel. Position is **fully reactive** via DataStar `data-style:--pan-x`/`data-style:--pan-y`/`data-style:--zoom-pct` expressions bound to `$pan`/`$tilt`/`$zoom` signals — no `app.js` update code needed. The server-rendered `style` attribute provides correct initial position before DataStar initializes (eliminates FOUC at 0,0).
...
- **Preset UI**: `presetSection(names)` template renders inside the PTZ card when online. Shows save input + button, preset count (N/16), and preset chips. Each chip has a load button (click name → `@post('/api/preset/load/{name}')`) and delete button (× → `@post('/api/preset/delete/{name}')`). Save uses DataStar `data-bind:presetName` + `data-on:click` with `@post('/api/preset/save/' + encodeURIComponent($presetName.trim()))` — survives DataStar panel morphs natively. `webStatus.PresetNames []string` is sorted in `getWebStatus()` from `state.Presets` map.
...
- **SSE connection indicator + offline banner**: `app.js` listens to DataStar's document-level `datastar-fetch` custom events (no element filter — any `datastar-patch-elements` event proves server reachability). On `datastar-patch-elements`/`datastar-patch-signals` (data flowing) → indicator shows green (`.connected`). On `started` → yellow (default, connecting). On `retrying`/`retries-failed` → red (`.disconnected`) + `#offline-banner` shows. Both `#sse-indicator` and `#offline-banner` are in `page()` OUTSIDE `#status-panel` so DataStar morphs don't reset their state.

### External Libraries Considered

- **`go-error-family`** (same author, `github.com/larsartmann/go-error-family` v0.10.0): **ADOPTED** for behavioral error classification. Registers all daemon sentinels with Families (Infrastructure/Rejection/Transient) so `errorfamily.HTTPStatus(err)` derives correct HTTP status codes, `errorfamily.ExitCode(err)` derives BSD sysexits exit codes, and `errorfamily.LogError()` adds family/code/retryable structured log fields with correct severity. Used in `stream.go` (7 typed stream errors via `NewInfrastructure()`, fixed 3x 500→503 bugs), `handlers.go` (preset validation HTTP status), `main.go` (CLI exit codes + structured logging at daemon init failure). Scoped adoption: DataStar action handlers return SSE patches with toast via `ExecuteScript` (correct DataStar pattern), NOT classified JSON errors. Circuit breaker logic untouched (more sophisticated than binary `IsRetryable`).
- **`datastar-go`** (`github.com/starfederation/datastar-go` v1.2.2): **ADOPTED** — Go SDK for DataStar. Provides `datastar.NewSSE(w, r)` for SSE event generation, `sse.PatchElementTempl(templ.Component)` for morphing DOM elements by ID, `sse.ExecuteScript()` for client-side toast dispatch, and `datastar.ReadSignals(r, &struct)` for reading client signals from POST bodies. The `TemplComponent` interface matches templ v0.3.x's `Component` interface. JS runtime: DataStar v1.0.2 (34 KB ES module, self-hosted at `/static/datastar.js`). Requires CSP `'unsafe-eval'` for expression evaluation.
- **`cqrs-htmx/v2`** (same author): HTMX-aware web infrastructure library. **Evaluated and REMOVED** — was briefly adopted for SSE, middleware, embedded HTMX JS, and WriteJSON, but pulled in ~30 transitive deps including a private repo (`go-cqrs-lite`) that broke `nix build`. The 8 used features were reimplemented locally in `sse.go`. HTMX itself was later replaced by DataStar (v1.0.2). All deps are now on the public Go module proxy.
