...
| `pixy_simulator_impl_test.go` | 31 tests for the simulator: config validation, commit sequencing, query round-trips (tracking/audio/gesture), failure injection (sendErr, sendRecvErr, nil response, corrupt response), daemon integration (setTracking/setAudio/setGesture → simulator → verify), syncState reads from simulator, protocol byte layout verification |
...
- **HID protocol simulator**: `pixy_simulator_test.go` provides `pixySimulator` — a protocol-faithful `HIDDevice` implementation that validates every outgoing byte against the PIXY wire protocol. Unlike `fakeHIDDevice` (which blindly records bytes), the simulator enforces config→commit sequencing, validates mode bytes per interface, and generates protocol-valid query responses that round-trip through `parseHIDResponse`. `withPixySimulator()` wires it into the daemon while keeping real `setTracking`/`setAudio`/`setGesture` methods (exercises the full `setDeviceState` → `Send` → protocol validation path, including the 200ms sleep). Supports failure injection: `sendErr`, `sendRecvErr`, `nilResponse` (simulates timeout), `corruptResp` (returns garbage). State accessors (`Tracking()`, `Audio()`, `Gesture()`) and recording (`SentReports()`, `Queries()`) enable test assertions.
