...
- **Git-derived version**: `package.nix` receives `version` from `flake.nix` as `self.shortRev or self.dirtyRev or "dev"`. No hardcoded version string. `doCheck = false` because `go-branded-id` v0.3.0 changed `String()` output (includes prefix like `"PID:42"` vs `"42"`) — pre-existing test failure. CI runs tests via GitHub Actions (`go test`), not nix.
