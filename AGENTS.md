...
nix fmt                                     # treefmt: nixfmt/templ/gofumpt — note templ regenerates *_templ.go
dprint fmt                                  # yaml/json/dockerfile in devShell (markdown deliberately ungated, see dprint.json)
...
- **V2 HID command families** (`internal/pixy/v2head.go`, `motor.go`, `identity.go`): official single-report commands — head `[0x09, iface, category, cmd]`; motor commands route the iface byte to 0x63 (motor-MCU sub-device, statically confirmed at a Beta.25 send site) and send NO commit pair. `speed`, `tracking`, `battery`, `preset push`/`pull`, and the identity queries ride them; battery answers are TTL-cached (60s) and every surface degrades by omission. EVIDENCE GRADES (v2head.go comments carry them, map doc §3.5a is the reference): head bytes statically evidenced (Mac 2.0.3 cmdtable; 108/162 confirmed byte-identical on Beta.25 x64 via `tools/emhid/extract_x64.py` — the old "version-shift" hypothesis is disproved, IDs match exactly); response framing DECODED + implemented (head-echo with the official `resp[1] & 0x1F` routing mask via `pixy.v2EchoMatches`, reserved dword, payload@8 via shared `pixy.V2ResponsePayloadOffset`, min len 9); `TargetTrackMode` corrected to the official 1-based scheme (0=none, 1=face, 2=halfbody, 3=fullbody — numeric CLI aliases removed); `ChargeStatus` typed ({1,2}=charging); preset GET answers are mode-only in the Beta.25 build (full `[slot][mode][pan][tilt][zoom]` rides the SET echo — both shapes parsed). Still ASSUMED: `MotorType` 0/1/2, DefaultPosMode values, speed unit, preset slot count, which preset-response shape the wired firmware answers — TODO_LIST #166 pins them on hardware.
...
- **Presets**: max 16 (`pixy.MaxPresets`), names validated by `pixy.ValidatePresetName`, persisted in `state.json` as `pixy.PresetMap`; `preset push` mirrors them into hardware slots (alphabetical mapping, assumed cap 8) and `preset pull` sweeps slots back as additive `hw-<slot>` presets (occupied = mode byte 1; never overwrites existing names; unreachable device aborts; isolated per-slot failures degrade the summary; the sweep EARLY-ABORTS after 3 consecutive slot failures — `presetPullMaxConsecutiveFailures`, success resets the counter — and says so in the summary; `--dry-run` runs the identical sweep report-only: no store/persist/broadcast). Pull is manual by design — no auto-sync of hardware slots on device appear (explicit decision, 2026-09-19). Web UI handles multi-word names; CLI `strings.Fields` truncates at the first space — ADR written (`docs/adr/2026-09-18_multi-word-preset-names.md`), join-remaining lands after Lars approves.
...
- Options: `withInCall()`, `withAutoOff()`, `withCameraInUse()`, `withNotify*()`, `withFindSource()`, `withCapture*()`, `withDebounceCount()`, `withPixySimulator(opts...)` (returns the simulator + option; `simulatorOption`s configure protocol state, e.g. `withPresetFullResponses()`).
- `pixySimulator` (`pixy_simulator_test.go`): protocol-faithful `HIDDevice` that validates every outgoing byte, enforces config→commit sequencing, generates round-trippable responses, and supports failure injection (`sendErr`, `commitErr` — the realistic breaker trigger, `sendRecvErr`, `nilResponse`, `corruptResp`). V2-head families live in `pixy_simulator_v2_test.go` + `pixyProtocolState` (payload validation, response builders — framed via `pixy.V2ResponsePayloadOffset`, failure-injection parity). V2 GET responses are returned at their EXACT payload length (real hidraw short-reads; parsers gate on received length) — do not assume 64-byte padded reads; preset-slot GETs answer mode-only by default, with the full SET-echo shape behind the `withPresetFullResponses()` builder option (mid-test flips: `sim.SetPresetFullResponses(on)` — never poke the state field directly); speed GET queries are modeled in BOTH evidenced forms — the 2.0.3 dedicated head `09 03 01 13` and the Beta.25 SET-head form `09 63 01 03` + motor byte (speed-query duality, pinned by `TestPixySimulatorV2_SpeedQueryDuality`). Prefer it over `fakeHIDDevice` for HID-path tests; `v2Response(head, payload...)` in `test_helpers_test.go` builds framed responses.
