...
| `ptz.go`           | PTZ logic: `ptzAxes` map, `handlePTZCommand`, `parsePTZValue`, `ptzAxisValue`/`ptzAxisValid`, V4L2 control name constants (extracted from handlers.go + v4l2.go)                                                                                                                |
...
| `middleware.go`    | Assigns middleware variables (`securityHeaderMiddleware`, `requestIDMW`) — all middleware implementations live in `sse.go` (reimplemented locally after cqrs-htmx removal)                                                                                                        |
| `sse.go`           | Local SSE + HTTP infrastructure (~315 lines): `Broadcaster` (thread-safe fan-out), `sseStream` (per-client SSE), `writeSSEEvent`/`splitSSELines`, `writeJSON`, `chain` (middleware chaining), `statusRecorder` (status capture + Flusher/Pusher), `securityHeadersMiddleware`, `requestIDMiddleware`, `loggingMiddlewareFactory` |
...
| `deps.go`          | `Dependencies` struct consolidating 10+ DI function pointers, `noopDependencies()` factory                                                                                                                                                                                      |
