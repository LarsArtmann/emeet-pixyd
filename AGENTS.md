...
| `middleware.go`    | cqrs-htmx middleware config (security headers, logging, request ID), caching FS                                                                                                                                                                                                |
...
| `static/`          | Frontend assets (app.js, style.css) — embedded via `//go:embed`. HTMX JS served separately via `cqrshtmx.HTMXScriptHandler()` at `/static/htmx.js`                                                                                                                             |
...
- `Daemon.broadcaster` (`*cqrshtmx.Broadcaster`) — thread-safe SSE fan-out with its own internal `sync.RWMutex`; no daemon-level lock needed for broadcasts
...
- **PTZ limits are hardware-verified**: `PanMin/Max = ±150°`, `TiltMin/Max = ±90°`, `ZoomMin/Max = 100–150`. These were verified empirically against the EMEET PIXY via `v4l2-ctl --list-ctrls` (`pan_absolute`/`tilt_absolute` range ±540000/±324000 at 3600 units/°, `zoom_absolute` 100–150). Tilt sign convention: **positive = up** everywhere (V4L2 read, V4L2 write, web slider, keyboard arrows) — no inversion anywhere.
...
- **Lint is clean (0 issues)**: ALL golangci-lint v2 linters are enabled in `.golangci.yml`. Specific false positives are suppressed via `//nolint` directives at call sites (e.g., `//nolint:exhaustruct` for intentional partial struct initialization, `//nolint:gochecknoglobals` for OTel metric vars and package-level middleware vars). Gosec excludes cover hardware-daemon patterns (G104, G107, G115, G204, G301, G304, G306, G702, G703, G706).
...
- **Handler extraction**: `handlers.go` (was 624 lines) extracted into `metrics.go` (OTel registration + updateMetrics), `stream.go` (MJPEG streaming + JPEG parsing), `middleware.go` (caching FS, cqrs-htmx middleware config). Handlers.go is now ~310 lines focused on HTTP routing and web handlers. Security headers, logging, request ID, Chain, WriteJSON, SSE, and HTMX JS serving all use `cqrs-htmx/v2` (same author's library).
...
- **Middleware uses cqrs-htmx**: `loggingMiddleware` (`cqrshtmx.RequestLoggingSlog`), `securityMiddleware` (`cqrshtmx.SecurityHeadersMiddlewareWithConfig`), `requestIDMiddleware` (`cqrshtmx.ContextEnrichmentMiddleware(nil)`) are configured in `middleware.go`. The chain is applied via `cqrshtmx.Chain(mws...)(handler)` in `main.go`. Request IDs are ULID-based (26 chars) via cqrs-htmx's context enrichment — not the previous 8-char hex format.
...
- **Live UI updates via SSE (cqrs-htmx)**: `/api/events` endpoint uses `cqrshtmx.NewSSEStream(w, r)` + `cqrshtmx.Broadcaster` for thread-safe fan-out. The handler sends a `connected` event on join, then forwards `refresh` events from the broadcaster. `app.js` connects on load, reconnects with exponential backoff, and closes the connection when the tab is hidden. HTMX JS is served at `/static/htmx.js` via `cqrshtmx.HTMXScriptHandler()` (embedded v2.0.9 with ETag/immutable caching) — no local static file.
- **State-change broadcasting via cqrs-htmx Broadcaster**: `broadcastStateChanged()` calls `d.broadcaster.Broadcast(cqrshtmx.SSEEvent{...})` after every state mutation (`setDeviceState`, `syncState`, `handleCallStart`, `handleCallEnd`, `applyProbeResultLocked`). The `Broadcaster` is a thread-safe fan-out hub with O(1) unsubscribe via channel pointer identity — race-safe, non-blocking sends drop to slow clients. The `Daemon.broadcaster` field (`*cqrshtmx.Broadcaster`) is initialized in both `NewDaemon()` and `newTestDaemon()`.
...
- **`vendorHash` is shared between `flake.nix` and `package.nix`**: Both files must be kept in sync when dependencies change. The hash is a SHA of module tarballs downloaded via `proxyVendor = true`, not of vendored source. **Nix build is currently broken** after cqrs-htmx migration: cqrs-htmx depends on `go-cqrs-lite` (`github.com/larsartmann/go-cqrs-lite`), which is a PRIVATE GitHub repo not on the Go module proxy. The nix FOD sandbox cannot access private repos. To fix: make go-cqrs-lite public, then update vendorHash. CI (`go-test.yml`) uses `GOPRIVATE` + `GITHUB_TOKEN` git config to access private deps.

### External Libraries

- **`cqrs-htmx/v2`** (same author): HTMX-aware web infrastructure library. **Adopted** for: SSE (`Broadcaster` + `SSEStream` replacing hand-rolled event client map), security headers (`SecurityHeadersMiddlewareWithConfig`), middleware chain (`Chain`), request logging (`RequestLoggingSlog`), request ID enrichment (`ContextEnrichmentMiddleware`), embedded HTMX JS (`HTMXScriptHandler` v2.0.9), and `WriteJSON`. NOT used for: CQRS dispatch, Casbin authz, CSRF, rate limiting, or `usermgmt` submodule (none fit a local-only hardware daemon). `httputil` remains as an indirect dependency (cqrs-htmx uses it for `ClientIP`).
