...
- **HID protocol simulator**: `pixy_simulator_test.go` provides `pixySimulator` — a protocol-faithful `HIDDevice` implementation that validates every outgoing byte against the PIXY wire protocol. Unlike `fakeHIDDevice` (which blindly records bytes), the simulator enforces config→commit sequencing, validates mode bytes per interface, and generates protocol-valid query responses that round-trip through `parseHIDResponse`. `withPixySimulator()` wires it into the daemon while keeping real `setTracking`/`setAudio`/`setGesture` methods (exercises the full `setDeviceState` → `Send` → protocol validation path, including the 200ms sleep). Supports failure injection: `sendErr` (all Sends), `commitErr` (commit-only — the realistic circuit breaker trigger since commit failures don't re-probe), `sendRecvErr`, `nilResponse` (timeout), `corruptResp` (garbage). State accessors (`Tracking()`, `Audio()`, `Gesture()`), recording (`SentReports()`, `SentTimestamps()`, `Queries()`), and `pixyProtocolState` as a `*` pointer. Key insight: only the commit failure path can naturally accumulate `hidFailCount` to threshold — config Send failures trigger `probeDevices()` which either resets the counter (device found) or nils `hidDev` (device not found).
