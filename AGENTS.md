...
- **PTZ relative-mode is hardware-dependent**: `parsePTZValue` treats any value with a leading `+`/`-` as **relative** (e.g. `tilt -30` = "current − 30"), reading current position via `d.deps.parsePTZ`. Negative absolute values are NOT expressible through the command API. Tests exercising relative PTZ MUST inject `withNoopParsePTZ()` (deterministic `{0,0,0}`) — otherwise they read real `/dev/video0` and flake on hardware state. `newPTZDaemon` wires this by default; tests using `newTestDaemon` directly must add it.
...
- **Handler extraction**: `handlers.go` (was 624 lines) extracted into `metrics.go` (OTel registration + updateMetrics), `stream.go` (MJPEG streaming + JPEG parsing), `middleware.go` (caching FS, httputil middleware config). Handlers.go is now ~310 lines focused on HTTP routing and web handlers. Security headers, logging, and request ID use `httputil` library directly — no local reimplementation.
...
- **Middleware uses httputil**: `loggingMiddleware`, `securityMiddleware`, `requestIDMiddleware` are `httputil.Middleware` vars configured via httputil factory functions (`httputil.Logging`, `httputil.SecurityHeaders`, `httputil.RequestID`). No local `responseWriter` or `Chain` wrapper — `httputil.Chain` is used directly in `main.go`.
...
- **Live UI updates via SSE**: `/api/events` endpoint streams `refresh` events over Server-Sent Events. `app.js` connects on load, reconnects with exponential backoff, and closes the connection when the tab is hidden. The status panel no longer polls every 3s; it refreshes only on explicit actions or SSE events.
- **State-change broadcasting**: `broadcastStateChanged()` notifies all active SSE clients after every state mutation (`setDeviceState`, `syncState`, `handleCallStart`, `handleCallEnd`, `applyProbeResultLocked`). Sends are non-blocking with a buffered channel so slow clients drop events without stalling the daemon.
