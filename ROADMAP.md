...
- **`preset pull` (hardware → state), TODO #141 design** — the inverse of `preset push`: sweep hardware motor slots into named software presets. The response format is now DECODED (Beta.25 parser disasm): `GET_MOTOR_PRESET_POS_MODE` (`09 63 01 17`) answers with mode u8 @offset 8 and pan/tilt/zoom dwords LE at 9/0xd/0x11 when mode==1 (min length 0x15). Design: query slots `1..N`, skip slots whose mode byte marks them invalid/empty, write each valid slot as a `hw-<slot>` preset (never overwriting user-named presets — pull is additive and explicit, no auto-sync). Remaining blockers: the #166 hardware session confirming the framing on-wire + the slot count + the mode-value semantics. Effort once pinned: S.
