...
- **usbmon cross-validation** — capture the official app on Windows to confirm the `mergeType` sub-device routing (`0x63` vs `3` iface byte; note: Beta.25 senders use iface `3` — the `0x63` substitution may be 2.0.3-only).
...
- **`preset pull` (hardware → state), TODO #141 — IMPLEMENTED (2026-09-19)**: the inverse of `preset push`: sweep hardware motor slots into named software presets. The response format is DECODED (Beta.25 parser disasm): `GET_MOTOR_PRESET_POS_MODE` (`09 63 01 17` + slot byte) answers with mode u8 @offset 8 and pan/tilt/zoom dwords LE at 9/0xd/0x11 when mode==1 (min length 0x15). Shipped exactly per this design: `preset pull` queries slots `1..8` (the assumed cap), skips slots whose mode byte is not 1, writes each occupied slot as an additive `hw-<slot>` preset (never overwriting user-named presets, no auto-sync), and gained a web Pull button (`POST /api/preset/pull`). Residual for the #166 hardware session: pin the real slot count (the sweep itself is the discovery tool) and confirm the mode-value semantics + framing on-wire.
