...
- **Motor-preset pull (`preset pull`, TODO #141)**: the inverse of push — a read-only sweep of hardware motor slots 1..8 over the official `GetMotorPresetPosMode` command, storing each occupied slot (position-mode byte 1) as an additive `hw-<slot>` preset rounded and clamped to the V4L2 limits. Existing preset names are never overwritten, an unreachable device aborts immediately, per-slot failures degrade the summary instead of aborting, and a full preset map stops the sweep without evicting anything. Web surface: `POST /api/preset/pull` + a header Pull button (no confirm gate — nothing moves). Slot count and per-slot mode semantics stay on the #166 hardware-verification list.
