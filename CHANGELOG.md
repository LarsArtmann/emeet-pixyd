...
- **Static protocol decodes from the Beta.25 x64 send/parse sites (TODO #150/#152)**: the motor-speed query rides the SET head with the dev byte computed as `mergeType(3,3)=0x63` at send time (the 0x63 routing is confirmed for Beta.25, not 2.0.3-only); every official response parser masks the echo's dev byte with `0x1F` (implemented as `pixy.v2EchoMatches`, so both echo forms validate); per-command response shapes and min lengths for the preset/power-on-default/speed families decoded instruction-by-instruction and recorded in map doc §3.5a; the version-shift hypothesis ("Beta.25 IDs differ from 2.0.3") is disproved by the exact-match artifact and retracted.
