...
- **Motor-preset pull (`preset pull`, TODO #141)**: the inverse of push — a read-only sweep of hardware motor slots 1..8 over the official `GetMotorPresetPosMode` command. The parser accepts both statically evidenced response shapes (map doc §3.5a): full `[slot][mode][pan][tilt][zoom]` answers land as additive `hw-<slot>` presets rounded and clamped to the V4L2 limits, while mode-only answers (what the Beta.25 GET parser reads) count as "set (position not exposed)". Existing preset names are never overwritten, an unreachable device aborts immediately, per-slot failures degrade the summary instead of aborting, and a full preset map stops the sweep without evicting anything. Web surface: `POST /api/preset/pull` + a header Pull button (no confirm gate — nothing moves). When every slot is unreadable the error reports the failure count ("preset pull: 8/8 slots unreadable: …") so the message says how much of the sweep died. Slot count and the live response shape stay on the #166 hardware-verification list.
