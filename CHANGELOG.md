...
- **Website deploy CI** (`website.yml`): a deploy job builds fresh, greps the changelog content before upload, and deploys via `npx firebase-tools` — self-skipping until the `FIREBASE_SERVICE_ACCOUNT` secret exists (`FIREBASE_DEPLOY_SETUP.md` documents the setup).
