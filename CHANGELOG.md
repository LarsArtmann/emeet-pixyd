...
- **Motor-preset mirroring (, TODO #141)**: named presets can be mirrored into the camera's hardware motor slots — the daemon moves each axis over the official  command and saves the position with . Slots map deterministically from the alphabetical preset list; the physical camera moves during this command.
