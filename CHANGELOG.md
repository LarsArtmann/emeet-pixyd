...
- **Motor-preset mirroring (`preset push`, TODO #141)**: named presets can be mirrored into the camera's hardware motor slots — the daemon moves each axis over the official `SetMotorPos` command and saves the position with `SetMotorPresetPos`. Slots map deterministically from the alphabetical preset list; the physical camera moves during this command.
- **Tracking-mode variants (`tracking` command, TODO #140)**: the official `SetTargetTrack` command adds mode-aware tracking on top of the binary track switch — `tracking <face|halfbody|fullbody>` over CLI and a segmented picker in the web UI (visible while tracking is active). Enum values follow the UI ordering and are marked for hardware verification.
...
- **Battery/charge status (`battery` command, TODO #139)**: read-only official GET heads (`09 00 00 02` battery, `09 00 00 06` charge) surface in the `battery` command, the `status` output, the Waybar tooltip + additive JSON field, and a web panel row — with a 60s TTL cache so an unanswered query costs at most one timed-out request per window, and every surface degrades by omitting the line. Whether the wired PIXY answers these heads is pending hardware verification.
- **Identity queries in `device` output (TODO #151)**: `device` gains `sn=`, `ver=`, `devver=`, and `func=` (raw `0x%08x` bitfield) when the official identity heads answer — per-head best-effort, fields omitted entirely when unanswered.
...
- **Speed persistence + move wiring (TODO #138)**: `speed` now persists per-axis to `state.json` (the `speeds` key is omitted entirely while unset) and is re-asserted over the official `SetMotorSpeed` head before every move path — PTZ axis moves, preset load/push, center, and reconcile-on-device-appear (fail-fast per re-assert so one flaky device cannot triple-count toward the HID circuit breaker). Web speed sliders initialize from the persisted state, and the `v4l2Mu → hidMu` lock order the wiring relies on is pinned by a mixed-workload deadlock-watchdog test.
- **Tracking-variant persistence (TODO #140)**: the chosen variant survives daemon restarts in `state.json`; the web picker shows the persisted variant instead of resetting to the default.
- **Preset-push web confirmation (TODO #141)**: `POST /api/preset/push/{name}` plus a push button on every preset chip, guarded by a browser confirm prompt (the command physically moves the camera).
- **Motor-preset pull (`preset pull`, TODO #141)**: the inverse of push — a read-only sweep of hardware motor slots 1..8 over the official `GetMotorPresetPosMode` command. The parser accepts both statically evidenced response shapes (map doc §3.5a): full `[slot][mode][pan][tilt][zoom]` answers land as additive `hw-<slot>` presets rounded and clamped to the V4L2 limits, while mode-only answers (what the Beta.25 GET parser reads) count as "set (position not exposed)". Existing preset names are never overwritten, an unreachable device aborts immediately, isolated per-slot failures degrade the summary, and a full preset map stops the sweep without evicting anything. Web surface: `POST /api/preset/pull` + a header Pull button (no confirm gate — nothing moves). When every attempted slot is unreadable the error reports the failure count (e.g. "preset pull: 3/8 slots unreadable, aborted after 3 consecutive failures: …") so the message says how much of the sweep died. Slot count and the live response shape stay on the #166 hardware-verification list.
- **Preset-pull early abort (TODO #167)**: the sweep stops after 3 consecutive slot failures (`presetPullMaxConsecutiveFailures`, matching the HID circuit breaker's consecutive-failure semantics) instead of grinding through the remaining slots — worst case drops from ~4s of dead air on an unresponsive device to ~0.75s. One success resets the counter (intermittent devices still get the full sweep), the summary carries an explicit "aborted after 3 consecutive failures" note so a truncated sweep is never mistaken for a complete one, and the unreadable-slot error reports attempted vs total ("3/8 slots unreadable").
- **`preset pull --dry-run` (TODO #171)**: a report-only sweep — identical slot queries and accounting, but nothing is stored, persisted to `state.json`, or broadcast to the web UI. The summary says what WOULD land ("preset pull (dry run): would pull: hw-3, 6 empty, 1 already named"), so users can preview a pull against their named presets before committing.
- **Pull additivity property test (TODO #169)**: `TestProperty_PresetPull_NeverEvictsOrMutates` runs 200 seeded randomized scenarios — random user presets including `hw-N` names colliding with hardware slots (and an out-of-range `hw-9`), random slot occupancy, random limit pressure — and asserts pull never evicts or mutates an existing entry, never exceeds `pixy.MaxPresets`, and only ever adds `hw-<slot>` names matching the seeded slot values. Fixed seed keeps CI failures reproducible.
- **Simulator fidelity knobs (TODO #168)**: `withPixySimulator` gains `simulatorOption`s — `withPresetFullResponses()` replaces the direct `presetFullResponses` field pokes (mid-test flips use the new `SetPresetFullResponses`) — and the simulator now models the speed-query duality: the Beta.25 form that rides the `SetMotorSpeed` head routed to `0x63` (`09 63 01 03` + motor byte) answers with the same `[motorType][speed][limit]` payload as the 2.0.3 dedicated GET head (`09 03 01 13`), so the #166 hardware session can compare either form against the wired firmware. Pinned by `TestPixySimulatorV2_SpeedQueryDuality`.
- **Static protocol decodes from the Beta.25 x64 send/parse sites (TODO #150/#152, closes #152)**: the motor-speed query rides the SET head with the dev byte computed as `mergeType(3,3)=0x63` at send time (the 0x63 routing is confirmed for Beta.25, not 2.0.3-only); every official response parser masks the echo's dev byte with `0x1F` (implemented as `pixy.v2EchoMatches`, so both echo forms validate); per-command response shapes and min lengths for the preset/power-on-default/speed families decoded instruction-by-instruction and recorded in map doc §3.5a. The version-shift hypothesis ("Beta.25 IDs differ from 2.0.3") is disproved by the exact-match artifact and retracted, and the cross-verification is complete: exhaustive scans for inline head constructions (dword-immediate, byte-wise, rip-relative) found zero additional sites, so the 53 Mac-only heads are absent from the x64 build — every head it contains agrees with the Mac table.
- **Battery/ADR polling design (TODO #139)**: `docs/adr/2026-09-19_battery-polling-on-demand-ttl.md` — on-demand reads with the 60s TTL stay; background refresh rejected until the hardware verdict, hybrid documented as the revisit trigger.
- **Windows x64 command-head extractor** (`tools/emhid/extract_x64.py` → `x64_heads.json`): sweeps the CRT initializer thunks of the Beta.25 Windows x64 build (re-acquired durably from the Wayback Machine into `~/specimens/emeet-studio/`) and extracts **108 command heads matching the macOS 2.0.3 `cmdtable.json` byte-for-byte, zero contradictions** (one zero-head artifact filtered; 53 Mac-only heads remain for the inline-sender completion path). Regeneration from the specimen is deterministic and was verified byte-identical to the committed artifact. The Beta.25 Inno parse data is committed alongside (`tools/inno661/data/parsed-beta25.json`). (2026-09-19 correction: the originally hypothesized version-shift model is retracted — the exact matches need no shift; see the protocol map.)

### Changed

- **CI hardening**: golangci-lint pinned to v2.13.2 (devShell parity), a fuzz-target existence assert diffs the repo's `Fuzz*` list against CI, the new `FuzzParseUevent` target covers both production parse shapes (00c0/0118 seeds) and found a real negative-index panic on its first run (see Fixed), and the dead `auto-tag.yml` (which could never fire on git-derived versions) is deleted.
- **dprint ownership decided — vendored into the devShell (TODO #170)**: `pkgs.dprint` joins the devShell so the previously orphaned `dprint.json` is honest again (a config no tool read). The config was trimmed to what this repo actually wants gated: the markdown plugin is dropped (its table re-padding would churn the deliberately compact TODO_LIST/report tables), and generated/owned files are excluded (`website/pnpm-lock.yaml` — pnpm + the frozen-lockfile guard own it — and `.config/**` machine metadata). `nix develop -c dprint check` passes clean; markdown stays deliberately ungated.
- **V2 response framing corrected — statically evidenced (TODO #150)**: official Beta.25 parser disassembly shows responses echo the 4-byte request head, carry a reserved dword at bytes 4..7, and place the payload at **offset 8** (minimum length 9); our parsers assumed offset 4 and would have misread every V2 response on real hardware. All read parsers now validate the head echo and framing via the shared `pixy.V2ResponsePayloadOffset` (the simulator builds responses with it too), so daemon, simulator, and evidence evolve together. Hardware confirmation stays on the #166 list.
- **Tracking enum corrected to the official 1-based scheme (TODO #150)**: Beta.25 UI-registration decode shows `0 = None ("No Smart Composition"), 1 = Face, 2 = HalfBody, 3 = FullBody` — our old 0-based assignment sent `none` where the user asked for `face`. The `none`/`off` variant is now exposed over CLI and as a web picker segment, persisted like the others; the misleading numeric CLI aliases were removed; the wire bytes are pinned by `TestTargetTrackModeWireValues`.
- **Charge status semantics decoded (TODO #150/#139)**: typed `pixy.ChargeStatus` — official consumer code computes charging as `sta ∈ {1,2}`, `0` discharging (the 1-vs-2 distinction is undecoded so far and both map to "charging"). Values outside the enum are rejected; the Waybar output now tags `charging`/`discharging` classes on the battery line so user CSS can style the states (pinned by `TestWaybarBatteryClass`).
- **Website deploy CI activated** (`website.yml`, TODO #133): the `FIREBASE_SERVICE_ACCOUNT` repository secret now exists (service account `github-website-deploy@lars-software` holding `roles/firebasehosting.admin`), so the deploy job runs on every `master` push touching `website/**` — fresh build, stale-content sentinel grep, then `npx firebase-tools deploy --only hosting:emeet-pixyd --project lars-software`. A manual `workflow_dispatch` trigger was added for re-deploys and verification. Create/rotate steps are in `FIREBASE_DEPLOY_SETUP.md` (first automated deploy: run 35437156913, green).
- **Repo/website metadata sweep**: the stale `htmx` GitHub topic swapped for `datastar`, the dead `minimumReleaseAgeExclude: html-validate@11.7.0` exclude removed, expiry notes added to the remaining pnpm release-age excludes, and the hero star metric hidden below 10 stars (neutral "Star on GitHub" instead).
