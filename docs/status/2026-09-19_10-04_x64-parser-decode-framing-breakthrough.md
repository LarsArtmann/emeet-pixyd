...
5. Dead-end string hunts before the method clicked: UTF-16 literals (none exist), qword pointer tables (none) — the actual mechanism was plain `lea rdx,[rip+…]` xrefs to string _starts_, which took three attempts to aim correctly (first xref computed from a match offset, not the string start).
...
17. Record the version-shift model in `docs/hid-protocol-official-map.md` (2.0.3 = Beta.25+2 insertions) — prevents future "discrepancy" confusion
18. Sweep the `CMD_*_VAL` string census into a doc table (command → response field names) — cheap now, useful at #166
19. Investigate response bytes 4–7 (unknown middle dword) at the #166 hardware session
20. Check whether 2.0.3 echoes the 0x63 iface in motor responses (dispatcher masks it; parsers compare raw — if echo is 0x63, official parsers would need masking too; Beta.25 senders use iface 3, so 0x63 may be 2.0.3-only) — #166 list
21. Purge `/tmp` scratch after the script lands (regenerable)
22. Time-box rule for research continuations: switch to deliverables at T+90min regardless of thread state
