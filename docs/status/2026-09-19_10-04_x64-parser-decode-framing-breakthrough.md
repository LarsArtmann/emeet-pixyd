...
1. **Apply the response-framing correction now?** Official x64 parsers read payload at **offset 8** (min length 9, head-echo); our parsers assume offset 4 — so on real hardware every V2 read path we have would parse garbage today. Applying = parsers + simulator framing + tests, marked "statically evidenced (Beta.25 parser disasm), hardware-unconfirmed". The counter-risk: no Mac-2.0.3 parser has been disassembled to confirm the same framing in the version our request heads target. Apply now, or bundle with the #166 hardware session?
2. **ChargeSta semantics into user surfaces now?** `{1,2}=charging, 0=discharging` is consumer-code-evidenced. Wiring it in changes `battery` output and adds Waybar charging/discharging classes (presentation only, degrades by omission if unanswered). Same evidence bar as Q1 — apply now or hold?
3. **Artifact storage for the x64 evidence** (concrete version of the carried specimen question): commit the sweep outputs (`x64_stubs_dedup.json`, ~109-head table + callback/slot map) and the Beta.25 `parsed.json` into `tools/emhid/` (~5 MB repo growth total), or keep all derived data beside the specimens in `~/specimens/` with only scripts + docs committed?
